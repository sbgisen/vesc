...
class CanPortConfig {
 public:
  ...
  const int& get_controller_id() const { return controller_id_; }
  const int& get_vesct_id() const { return vesct_id_; }
...
class CanPort {
 public:
  CanPort(const IoContext& ctx, const std::string& device_name,
 ...
 private:
  void async_send_handler(const asio::error_code& error,
